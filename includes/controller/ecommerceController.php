...
include("mpesaRequestController.php");
...
switch ($q){
    ...
    case "checkout":
		// $mpesaResponse = mpesaSTKPush('LB072100014', 'order_master','11', '254708374149', 254708374149  $dbClass);
		//mpesaSTKPush('LB072100014', 'order_master',$total_order_amt, $mpesa_mobile, $dbClass);
		//die;					
		...
		if(is_numeric($return_master) && $return_master>0){
            ...
            if(is_numeric($return_details) && $return_details>0 ){
                ...
                else  if(!isset($_SESSION['group_master']) && $payment_method==5){
					// $mpesaResponse = mpesaSTKPush('LB072100014', 'order_master','11', '254708374149', 254708374149  $dbClass);
					mpesaSTKPush($invoice_no, 'order_master',$total_order_amt, $mpesa_mobile, $dbClass);
					$cart = array();
                    $_SESSION['latest_order_id'] = $return_master;
                    //unblock flowing line after check
                    $_SESSION['cart'] 	= $cart;
	...
	case "get_order_amount":
        $result = $dbClass->getSingleRow("SELECT total_order_amt FROM  order_master where invoice_no='".$invoice_no."'");
		echo $result['total_order_amt'];	
	break;
    ...
    case "make_payment":
        if($payment_method==2){
            ...
            if (strpos($data['item_number'], 'LBG') !== false) {
                $return = $dbClass->update("group_order", $columns_value, $condition_array);
            }
        }
		else if($payment_method==5){
			$response = mpesaSTKPush($order_no, 'order_master',$paid_amount, $mpesa_mobile, $dbClass);
			echo $response;
		}
        break;
...
//this portion will remove while final submission......
    ...
    case "test_mail":
        ...
        $sql = "SELECT m.order_id, m.customer_id, 
                    ...
                    case payment_method when 1 then 'cash' when 2 then 'loyalty_point' when 3 then 'Paypal'  else 'Mpesa'  end payment_method
                    FROM order_master m
                    LEFT JOIN order_details d ON d.order_id = m.order_id
                    LEFT JOIN customer_infos c ON c.customer_id = m.customer_id
                    LEFT JOIN items p ON p.item_id = d.item_id
